...
# Center the first pie chart
col1, col2, col3 = st.columns([2, 2, 2])  # Three columns, with the middle one being twice as wide
with col2:
   # Visualization for the selected user's investment by instrument
...
# Attempt to create more space between the next two pie charts
# Increasing the number of spacer columns and adjusting the layout for more spacing
col_left, spacer1, spacer2, spacer3, spacer4, spacer5, spacer6, col_right = st.columns([20, 5, 3, 1, 1, 5, 5, 20])  # Adjusted for more spacing

with col_left:  # Left pie chart
    st.header('Return Based Rebalancing')  # Changed from st.subheader() to st.header()
...
with col_right:  # Right pie chart
    st.header('Risk Based Rebalancing')  # Changed from st.subheader() to st.header()
